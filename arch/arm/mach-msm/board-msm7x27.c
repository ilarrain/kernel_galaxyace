...
#if ( defined(CONFIG_MACH_CALLISTO) || defined(CONFIG_MACH_COOPER) )
	"ldo3"
...
static int msm_fb_lcdc_power_save(int on)
{
	...
	for (i = 0; i < ARRAY_SIZE(msm_fb_lcdc_vreg); i++) {
		if (on) {
			...
			tmp = gpio_tlmm_config(GPIO_CFG(GPIO_OUT_101, 0,
...
#if ( defined(CONFIG_MACH_CALLISTO) || defined(CONFIG_MACH_BENI) )
...
#if ((defined CONFIG_S5K4ECGX) || (defined CONFIG_S5K4ECGX_COOPER))
